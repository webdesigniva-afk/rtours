  ...
  for (const item of items) {
    ...
    hotels.set(key, {
      ...
      raw: {
        ...
        images: asArray(valueByNames(item, ["images", "Images"])),
        description: textFromHtml(valueByNames(item, ["description", "Description"])) || null
      }
    });
  }
...
function collectSupplierEntities(program, pricesPayload, media, dates, itinerary, services) {
  ...
  if (terms) entities.push({ type: "useful_info", key: "terms", title: "Условия и важна информация", sortOrder: 0, raw: { text: terms } });
...
function mapAbaxProgram(program, pricesPayload) {
  ...
  return {
    ...
    title,
    summary: textFromHtml(valueByNames(program, ["short_description", "ShortDescription", "summary", "Summary"])) || description || null,
    description,
    country,
    region,
    city: region,
...
function mapAbaxProgramV2(program, pricesPayload) {
  ...
  return {
    ...
    title,
    summary: intro || publicDescription || null,
