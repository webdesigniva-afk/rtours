...
function businessDateSnapshot(date, index) {
  return {
    key: dateKey(date, index),
    label: compactValue(date?.label),
    startDate: compactValue(date?.startDate),
    endDate: compactValue(date?.endDate),
    departurePoints: compactValue(date?.departurePoints),
    availability: compactValue(date?.availability),
    priceFrom: compactValue(date?.priceFrom),
    currency: compactValue(date?.currency),
    seatsTotal: compactValue(date?.seatsTotal),
    seatsAvailable: compactValue(date?.seatsAvailable)
  };
}

function businessMediaSnapshot(media, index) {
  return {
    key: mediaKey(media, index),
    url: compactValue(media?.url),
    alt: compactValue(media?.alt)
  };
}

function businessItinerarySnapshot(day, index) {
  return {
    key: itineraryKey(day, index),
    dayNumber: compactValue(day?.dayNumber || index + 1),
    title: compactValue(day?.title),
    description: compactValue(day?.description),
    accommodation: compactValue(day?.accommodation),
    meals: compactValue(day?.meals),
    transport: compactValue(day?.transport)
  };
}

function businessOfferSnapshot(offer) {
  return {
    externalId: compactValue(offer.externalId),
    productType: compactValue(offer.productType),
    productTypeLabel: compactValue(offer.productTypeLabel),
    title: compactValue(offer.title),
    summary: compactValue(offer.summary),
    description: compactValue(offer.description),
    country: compactValue(offer.country),
    region: compactValue(offer.region),
    city: compactValue(offer.city),
    durationDays: compactValue(offer.durationDays),
    durationNights: compactValue(offer.durationNights),
    transport: compactValue(offer.transport),
    priceFrom: compactValue(offer.priceFrom),
    currency: compactValue(offer.currency),
    heroImageUrl: compactValue(offer.heroImageUrl),
    dates: arrayValue(offer.dates).map(businessDateSnapshot).sort((a, b) => String(a.key).localeCompare(String(b.key))),
    media: arrayValue(offer.media).map(businessMediaSnapshot).sort((a, b) => String(a.key).localeCompare(String(b.key))),
    itinerary: arrayValue(offer.itinerary).map(businessItinerarySnapshot).sort((a, b) => String(a.key).localeCompare(String(b.key))),
    highlights: normalizeTextList(offer.highlights),
    includedServices: normalizeTextList(offer.includedServices),
    excludedServices: normalizeTextList(offer.excludedServices)
  };
}
...
function buildImportantChanges(previous, next) {
  ...
  const fields = [
    ["content", "productType", previousOffer.productType, next.productType],
    ["content", "productTypeLabel", previousOffer.productTypeLabel, next.productTypeLabel],
    ["content", "title", previousOffer.title ?? previous.title, next.title],
    ["content", "summary", previousOffer.summary, next.summary],
    ["content", "description", previousOffer.description, next.description],
    ["destination", "country", previousOffer.country, next.country],
    ["destination", "region", previousOffer.region, next.region],
    ["destination", "city", previousOffer.city, next.city],
...
export async function upsertSupplierOffer(client, inputOffer, options = {}) {
  ...
  const checksum = createHash("sha256").update(stablePayload(businessOfferSnapshot(offer))).digest("hex");
  ...
  const changeState = !existingRow
    ? "new"
    : shouldForceRewrite || importantChanges.length > 0
      ? "changed"
      : "unchanged";
