...
export async function upsertSupplierOffer(client, inputOffer, options = {}) {
  ...
  const existing = await client.query(
    `
      select import.offer_id, import.checksum, import.normalized_payload, offer.title, offer.price_from, offer.currency,
        offer.duration_days, offer.duration_nights, offer.transport
  ...
  await replaceRelations(client, offerId, offer, provider);
    ...
    await client.query(
      `
        ...
        values ($1, $2, $3, $4, $5, $6, nullif($7, ''), nullif($8, ''), nullif($9, ''),
          $10, $11, $12, $13, $14, $15::jsonb, $16, nullif($17, ''), nullif($18, ''), $19::jsonb)
      `,
      [
        importId,
        offerId,
        ...
        previousReview?.editorial_title || "",
        previousReview?.editorial_url || "",
