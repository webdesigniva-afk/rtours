...
function withoutInternalBohemiaMetadata(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return asObject(value);
  const { __bohemiaRaw, __bohemiaSearchResponse, ...publicData } = value;
  return publicData;
}
...
function mapRates(details, currency, durationDays, fallbackDeparturePoints = null) {
  ...
  return [...byDate.values()].map((item, index) => ({
    ...
    notes: [
      item.raw.hotels.length ? `Хотели: ${item.raw.hotels.join(", ")}` : null,
      item.raw.rooms.length ? `Стаи: ${item.raw.rooms.join(", ")}` : null
...
function mapHotels(details) {
  ...
  if (rateHotels.length) return rateHotels;
...
function mapBohemiaOffer(kind, searchResult, detailRoot, baseUrl) {
  const searchRaw = asObject(searchResult).__bohemiaSearchResponse || null;
  const detailRaw = asObject(detailRoot).__bohemiaRaw || null;
  const cleanSearchResult = withoutInternalBohemiaMetadata(searchResult);
  const cleanDetailRoot = withoutInternalBohemiaMetadata(detailRoot);
  const details = asObject(cleanDetailRoot?.ExcursionDetails || cleanDetailRoot?.HolidayDetails || cleanDetailRoot || {});
  const main = asObject(details.MainDetails || details);
  const externalId = firstValue(main.OfferID, cleanSearchResult.OfferID);
  const title = firstValue(main.OfferTitle, cleanSearchResult.OfferTitle, main.ShortTitle);

  if (!externalId || !title) return null;

  const currency = normalizeCurrency(main.Currency ?? cleanSearchResult.Currency);
  ...
  const trips = rateTrips.length ? rateTrips : collectTrips(cleanSearchResult, details, currency, departurePoints);
  ...
  }, toNumber(cleanSearchResult.Price));
  const tagline = firstValue(main.OfferTitleTagline, main.ShortTitleTagline, cleanSearchResult.OfferTitleTagline);
  ...
  const durationDays = firstInteger(main.Duration, main.Days, main.DaysCount, cleanSearchResult.Duration, cleanSearchResult.Days);
  const durationNights = firstInteger(main.Nights, main.NightsCount, main.Overnights, cleanSearchResult.Nights, cleanSearchResult.Overnights);

  const offer = {
    raw: {
      kind,
      searchResult: cleanSearchResult,
      details,
      bohemiaApi: {
    ...
    description,
    country: normalizeCountryName(firstValue(main.PrimaryDestinationCountryID, cleanSearchResult.PrimaryDestinationCountryID, destinations[0])) || "Бохемия",
    region: route || destinations.join(", ") || null,
    city: null,
    durationDays,
    durationNights: inferredDurationNights(durationDays, durationNights),
    transport: normalizeTransport(main.TransportationType ?? cleanSearchResult.TransportationType),
...
export function createBohemiaClient({ baseUrl, username, password, timeoutMs = 15000 }) {
  ...
  return {
    async searchExcursions(params = {}) {
      ...
      return asArray(asObject(payload.Resultset).Result).map((result) => ({
        ...asObject(result),
        __bohemiaSearchResponse: payload.__bohemiaRaw
      }));
    },
