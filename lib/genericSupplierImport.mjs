...
function inferDurationNights(days, nights) {
...
function displayDate(value) {
  const date = cleanDate(value) || cleanString(value);
...
function mapDepartures(item, mapping, currency) {
  return asArray(getFirstPath(item, mapping.datesItems)).map((date, index) => {
    ...
    const label = cleanString(getFirstPath(raw, mapping.dateLabel || ["label", "title", "name"])) || displayDate(startDate) || `Departure ${index + 1}`;

    const durationDays = cleanInteger(getFirstPath(item, mapping.durationDays || ["durationDays", "days", "Days", "duration", "Duration", "DaysCount"]));
    const durationNights = cleanInteger(getFirstPath(item, mapping.durationNights || ["durationNights", "nights", "Nights", "overnights", "Overnights", "NightsCount"]));
