          ...
          <div className="offer-new-full-field">
            ...
            <div className="offer-new-editor">
              <div className="offer-new-editor-toolbar">
                <button className={editorPanel === "format" ? "is-active" : ""} type="button" onMouseDown={keepEditorSelection} onClick={() => openEditorPanel("format")}>Paragraph <ChevronDown size={15} aria-hidden="true" /></button>
              ...
              {editorPanel ? <div className="offer-new-editor-panel">{editorPanel === "format" ? <><button type="button" onMouseDown={keepEditorSelection} onClick={() => { runEditorCommand("formatBlock", "p"); setEditorPanel(null); }}>Paragraph</button><button type="button" onMouseDown={keepEditorSelection} onClick={() => { runEditorCommand("formatBlock", "h2"); setEditorPanel(null); }}>Heading 2</button><button type="button" onMouseDown={keepEditorSelection} onClick={() => { runEditorCommand("formatBlock", "h3"); setEditorPanel(null); }}>Heading 3</button><button type="button" onMouseDown={keepEditorSelection} onClick={() => { runEditorCommand("formatBlock", "blockquote"); setEditorPanel(null); }}>Quote</button></> : null}{editorPanel === "link" ? <div className="offer-new-editor-inline-form"><input value={linkUrl} onChange={(event) => setLinkUrl(event.target.value)} placeholder="https://..." /><button type="button" onClick={applyLink}>Добави линк</button></div> : null}{editorPanel === "image" ? <div className="offer-new-editor-inline-form"><input value={imageUrl} onChange={(event) => setImageUrl(event.target.value)} placeholder="URL на изображение" /><button type="button" onClick={applyImage}>Добави снимка</button></div> : null}{editorPanel === "more" ? <><button type="button" onMouseDown={keepEditorSelection} onClick={() => runEditorCommand("removeFormat")}>Изчисти форматирането</button><button type="button" onMouseDown={keepEditorSelection} onClick={() => runEditorCommand("insertHorizontalRule")}>Разделител</button><button type="button" onMouseDown={keepEditorSelection} onClick={() => runEditorCommand("formatBlock", "blockquote")}>Цитат</button></> : null}</div> : null}
