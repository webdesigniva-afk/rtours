...
function createCountryCodeByName() {
  const displayNames = new Intl.DisplayNames(["bg"], { type: "region" });

  const countryCodesByName = isoCountryCodes.reduce<Record<string, string>>((result, code) => {
    const countryName = displayNames.of(code);
    if (countryName) {
      result[normalizeCountryKey(countryName)] = code.toLowerCase();
    }
    return result;
  }, {});

  for (const [name, code] of Object.entries(countryCodeAliases)) {
    countryCodesByName[normalizeCountryKey(name)] = code;
  }

  return countryCodesByName;
}

const countryCodeByName = createCountryCodeByName();

function getCountryFlagUrl(country: string) {
  const trimmedCountry = country.trim();
  const code = /^[A-Z]{2}$/i.test(trimmedCountry)
    ? trimmedCountry.toLowerCase()
    : countryCodeByName[normalizeCountryKey(trimmedCountry)];
